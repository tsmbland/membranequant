...
#
# # v = interpolate(tf.Variable(np.r_[np.arange(0, 10), 10], dtype=tf.float64),
# #                 tf.Variable(np.linspace(2, 8, 8), dtype=tf.float64), 3, False)
#
# x = [i for i in range(0, 128, 8)]
# x.append(127)
# x = np.asarray(x, np.float32)
#
# y = np.asarray([np.sin(2 * np.pi * t / 128) for t in x], np.float32)
#
# print(x)
# print(y)
#
# k = np.concatenate((np.reshape(x, (1, -1)), np.reshape(y, (1, -1))), 0)
# k = np.reshape(k, (1, k.shape[0], k.shape[1]))
# p = np.asarray(np.reshape(np.arange(0, 128), (1, 1, -1)), np.float32)
# spline_op = interpolate(knots=tf.constant(k), positions=tf.constant(p), degree=3, cyclical=False)
# #
