...
# Input data
conds_list_total = ['180301_NWG91_25PKC_Florent3',
...
def segment(direc):
    try:
        data = Data(direc)
        img = composite(data, settings=settings, factor=0.2, mag=mag, coors=data.ROI)
...
nwg91_rd = batch_import(adirec, np.array(conds_list_total)[[0, 1, 2]], Res)
