...
# Input data
conds_list_total = [
    '180607_nwg91_wt_tom4,15,30',
    '180611_nwg91_24hrchin1_tom4,15,30',
    '180611_nwg91_24hrpar1_tom4,15,30',
    '180611_nwg91_24hrspd5_tom4,15,30',
...
def segment(direc):
    try:
        data = Data(direc)
        img = af_subtraction(composite(data, settings=settings, factor=3, mag=1, coors=data.ROI), data.AF,
                             settings=settings)
...
alldata = batch_import(adirec, np.array(conds_list_total)[:], Res)
wt = batch_import(adirec, np.array(conds_list_total)[[0]], Res)
chin1 = batch_import(adirec, np.array(conds_list_total)[[1, 4]], Res)
par1 = batch_import(adirec, np.array(conds_list_total)[[2, 5]], Res)
spd5 = batch_import(adirec, np.array(conds_list_total)[[3]], Res)
